...
/**
 * The default implementation of {@link JMessageService}.
 * 
 * @author alex
 * 
 */
public class JMessageServiceImpl implements JMessageService {
   ...
   */
  @Inject
  public JMessageServiceImpl(@Stdout PrintWriter stdout) {
    super();
    this.stdout = stdout;
  }
   ...
   * comma-separated strings and {@link uk.co.unclealex.music.files.JFileLocation} into readable paths.
   * 
   * @author alex
   * 
   */
  class PrintableFunction implements Function<Object, Object> {
    ...
    public Object apply(Object input) {
      ...
      else if (input instanceof JFileLocation) {
        return ((JFileLocation) input).resolve().toString();
      }
      else if (input instanceof JUser) {
        return ((JUser) input).getName();
      }
      else {
        return input;
      }
