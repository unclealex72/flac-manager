 ...
 * 
 */
public interface JDeviceConnectionService {
   ...
   */
  public JScsiId findScsiIdForUuid(String uuid);

  /**
   * Find the mount point for a SCSI device.
   * 
   * @param scsiId
   *          The {@link uk.co.unclealex.music.sync.scsi.JScsiId} of the device to find.
   * @return The path where the SCSI device is mounted or null if it is not.
   */
  public Path findMountPointForScsiId(JScsiId scsiId);
}
