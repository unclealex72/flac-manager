 ...
 * 
 */
public class JFileLocation extends JDataObject implements Comparable<JFileLocation> {
   ...
   *          the read only
   */
  public JFileLocation(Path basePath, Path relativePath, boolean readOnly) {
    super();
   ...
   *          the more
   */
  public JFileLocation(boolean readOnly, Path basePath, String first, String... more) {
    this(basePath, Paths.get(first, more), readOnly);
  }

  /**
   * Compare this {@link JFileLocation} to another by comparing the results of
   * their.
   * 
   * @param o
   *          The {@link JFileLocation} to compare against.
   * @return See {@link Comparable#compareTo(Object)}. {@link #resolve()}
   *         method.
   */
  @Override
  public int compareTo(JFileLocation o) {
   ...
   * @return A new {@link JFileLocation} pointing to the new path.
   */
  public JFileLocation resolve(Path path) {
    return new JFileLocation(getBasePath(), getRelativePath().resolve(path), isReadOnly());
  }
