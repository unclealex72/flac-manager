 ...
 * 
 */
public class JDirectoryServiceImpl implements JDirectoryService {

  /**
   * The {@link uk.co.unclealex.music.message.JMessageService} used to indicate that files have been found.
   */
  private final JMessageService messageService;
  
  @Inject
  public JDirectoryServiceImpl(JMessageService messageService) {
    super();
   ...
   */
  @Override
  public SortedSet<JFileLocation> listFiles(JFileLocation requiredBasePath, Iterable<? extends Path> directories)
      throws JInvalidDirectoriesException,
      IOException {
    ...
    if (!Iterables.isEmpty(invalidPaths)) {
      throw new JInvalidDirectoriesException("The following paths are either not directories or not a subpath of "
          + absoluteRequiredBasePath, Iterables.filter(invalidPaths, Path.class));
    }
    SortedSet<JFileLocation> allFileLocations = Sets.newTreeSet();
   ...
   */
  @Override
  public SortedSet<JFileLocation> listFiles(final Path basePath) throws IOException {
    final SortedSet<JFileLocation> fileLocations = Sets.newTreeSet();
    final JMessageService messageService = getMessageService();
    FileVisitor<Path> visitor = new SimpleFileVisitor<Path>() {
      @Override
      public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
        JFileLocation fileLocation = new JFileLocation(basePath, basePath.relativize(file), Files.isWritable(file));
        fileLocations.add(fileLocation);
        messageService.printMessage(JMessageService.FOUND_FILE, fileLocation);
        return super.visitFile(file, attrs);
      }
    };
    Files.walkFileTree(basePath, visitor);
