 ...
 * @author alex
 */
public class JMusicBrainzClientImpl implements JMusicBrainzClient {

  private static final Logger log = LoggerFactory.getLogger(JMusicBrainzClientImpl.class);
  ...
  /**
   * The {@link JMusicBrainzWebResourceFactory} used to talk to MusicBrainz with
   * Jersey.
   */
  private final JMusicBrainzWebResourceFactory musicBrainzWebResourceFactory;
   ...
   */
  @Inject
  public JMusicBrainzClientImpl(JMusicBrainzWebResourceFactory musicBrainzWebResourceFactory) {
    super();
   ...
   */
  protected WebResource releases(JUser user, String collectionId) {
   ...
   */
  private void alterReleases(JUser user, Iterable<? extends String> releaseIds, WebResourceAction action) throws JNoCollectionException {
   ...
   */
  public ImmutablePair<String, Integer> findUsersCollection(JUser user) throws JNoCollectionException {
    ...
    if (size == 0) {
      throw new JNoCollectionException("No collections found.");
    }
    else if (size == 1) {
      ...
      if (collectionElement == null) {
        throw new JNoCollectionException("Cannot find a unique collection.");
      }
    }
  ...
  /**
   * Gets the {@link JMusicBrainzWebResourceFactory} used to talk to MusicBrainz with Jersey.
   *
   * @return the {@link JMusicBrainzWebResourceFactory} used to talk to MusicBrainz with Jersey
   */
  public JMusicBrainzWebResourceFactory getMusicBrainzWebResourceFactory() {
